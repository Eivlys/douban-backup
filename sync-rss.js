...
(async () => {
    ...
    feed.forEach((item) => {
        ...
        let comment = contents.filter((el) =>
            el.textContent.startsWith('备注')
        );
        if (comment.length) {
...
async function fetchItem(link, category) {
    ...
    if (category === CATEGORY.movie) {
        ...
        itemData[DB_PROPERTIES.POSTER] = dom.window.document
            ...
            ?.src.replace(/\.webp$/, '.jpg');
        const gameInfo = dom.window.document.querySelector(
            '#content .game-attr'
        );
        const dts = [...gameInfo.querySelectorAll('dt')].filter(
            (i) =>
                i.textContent.startsWith('类型') ||
                i.textContent.startsWith('发行日期')
        );
        if (dts.length) {
            dts.forEach((dt) => {
    ...
    try {
        ...
        if (properties[DB_PROPERTIES.POSTER]) {
            ...
            postData.cover = {
                type: 'external',
                external: {
                    url: properties[DB_PROPERTIES.POSTER]?.files[0]?.external
                        ?.url, // cannot be empty string or null
                },
            };
        }
